...
const FOCUS_IDS:Record<Exclude<ForgeFocus,'adaptive'>,string[]>={
  upper:['bench','row','ohp','pulldown','incline-db','curl','pullup','pushup'],
  lower:['squat','rdl','split-squat','leg-press','ham-curl','calf','lunge'],
  full:['squat','bench','row','rdl','ohp','carry','pushup','lunge'],
};
function isUpper(id:string){return FOCUS_IDS.upper.includes(id)}
function isLower(id:string){return FOCUS_IDS.lower.includes(id)}
function recentBias(intelligence?:TrainingIntelligence):Exclude<ForgeFocus,'adaptive'>|null{if(!intelligence?.recentResistanceMix.length)return null;const recent=intelligence.recentResistanceMix.slice(0,3);const upperCount=recent.filter(id=>['push','pull','ai-upper'].includes(id)).length;const lowerCount=recent.filter(id=>['legs','ai-lower'].includes(id)).length;if(upperCount>=2)return'lower';if(lowerCount>=2)return'upper';return null}
function chooseAdaptiveFocus(snapshot:ProgressionSnapshot,intelligence?:TrainingIntelligence):Exclude<ForgeFocus,'adaptive'>{const historyChoice=recentBias(intelligence);if(historyChoice)return historyChoice;const endurance=snapshot.staminaXP+snapshot.agilityXP;if(snapshot.thisWeekResistanceWorkouts>=3&&snapshot.thisWeekRecoverySessions===0)return'full';if(snapshot.strengthXP>endurance*1.35&&snapshot.workoutCount>2)return'lower';return snapshot.resistanceWorkoutCount%2===0?'upper':'lower'}
function projectedSessionXP(sets:number,minutes:number,streakDays:number){return calculateSessionXP({completedSets:sets,durationMinutes:minutes,prCount:0,streakDays})}
...
export function generateWorkoutPlan(args:{targetXP:number;focus:ForgeFocus;snapshot:ProgressionSnapshot;intelligence?:TrainingIntelligence;equipment?:EquipmentId[];lastSetsByExercise?:Record<string,LastWorkingSets>}):GeneratedWorkoutPlan{
  ...
  let desiredSets=6;
  for(let sets=6;sets<=18;sets+=1){desiredSets=sets;if(projectedSessionXP(sets,estimatedMinutes,args.snapshot.streakDays)>=targetXP)break;}
