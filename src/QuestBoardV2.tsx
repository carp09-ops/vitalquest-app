...
const questIcons: VQIconName[] = ['strength','trophy','agility'];
  ...
  return <SafeAreaView style={[styles.safe,{backgroundColor:t.background}]}>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.page,wide&&styles.pageWide]}>
      <View style={styles.shell}>
        <View><Text style={[styles.eyebrow,{color:t.accent}]}>QUEST BOARD</Text><Text style={[styles.title,{color:t.text}]}>Choose the next objective.</Text><Text style={[styles.lede,{color:t.muted}]}>Quest chains turn consistency, volume and mileage into visible progression and permanent rewards.</Text></View>

        <View style={[styles.tabs,{borderColor:t.border,backgroundColor:t.surface}]}>
          {(['Active','Completed','Guild'] as QuestTab[]).map(item=>{
            const active=item===tab;
            return <Pressable key={item} onPress={()=>setTab(item)} style={[styles.tab,active&&{backgroundColor:t.surfaceElevated,borderColor:t.accentSoft}]}><Text style={[styles.tabText,{color:active?t.accent:t.muted}]}>{item.toUpperCase()}</Text></Pressable>;
          })}
        </View>
          ...
          <View style={[styles.campaign,{borderColor:t.border,backgroundColor:t.surface}]}>
            <ImageBackground source={{uri:art}} resizeMode="cover" style={[styles.campaignArt,wide&&styles.campaignArtWide]} imageStyle={{opacity:.95}}>
              ...
              <View style={styles.campaignCopy}>
                <Text style={[styles.eyebrow,{color:t.accent}]}>MAIN QUEST · CHAPTER I</Text>
                <Text style={[styles.campaignTitle,{color:t.text}]}>Forge a Stronger You</Text>
                <Text style={[styles.campaignText,{color:t.text}]}>Complete five cornerstone objectives across training, movement, recovery and consistency.</Text>
              </View>
            </ImageBackground>
            <View style={styles.campaignHud}>
              <View style={styles.campaignMeta}><View><Text style={[styles.metaLabel,{color:t.muted}]}>OBJECTIVES</Text><Text style={[styles.metaValue,{color:t.text}]}>3 / 5</Text></View><View><Text style={[styles.metaLabel,{color:t.muted}]}>REWARD</Text><Text style={[styles.metaValue,{color:t.accent}]}>THE FORGED</Text></View><IconArt name="trophy" size={50}/></View>
              <View style={[styles.track,{backgroundColor:t.surfaceElevated}]}><View style={[styles.fill,{width:'60%',backgroundColor:t.accent}]}/></View>
              <Text style={[styles.progressCopy,{color:t.muted}]}>60% complete · 2 objectives remain</Text>
            </View>
          </View>

          <View style={styles.sectionHead}><View><Text style={[styles.eyebrow,{color:t.accent}]}>ACTIVE OBJECTIVES</Text><Text style={[styles.sectionTitle,{color:t.text}]}>Earn the next unlock.</Text></View><Text style={[styles.count,{color:t.muted}]}>{quests.length} ACTIVE</Text></View>

          <View style={[styles.questGrid,wide&&styles.questGridWide]}>
            {quests.map((quest,index)=>{
              const ratio=Math.min(1,quest.progress/quest.target);
              return <View key={quest.title} style={[styles.questCard,{borderColor:t.border,backgroundColor:t.surface}]}>
                <View style={styles.questHeader}><IconArt name={questIcons[index]??'quest'} size={48}/><View style={{flex:1}}><Text style={[styles.questType,{color:t.accent}]}>{index===0?'WEEKLY TRIAL':'MILESTONE'}</Text><Text style={[styles.questTitle,{color:t.text}]}>{quest.title}</Text></View></View>
                <Text style={[styles.questDescription,{color:t.muted}]}>{quest.description}</Text>
                <View style={styles.questNumbers}><Text style={[styles.questCurrent,{color:t.text}]}>{quest.progress}</Text><Text style={[styles.questTarget,{color:t.muted}]}>/ {quest.target}</Text><Text style={[styles.reward,{color:t.accent}]}>{quest.reward}</Text></View>
                <View style={[styles.track,{backgroundColor:t.surfaceElevated}]}><View style={[styles.fill,{width:`${ratio*100}%`,backgroundColor:index===0?t.positive:t.accent}]}/></View>
                <Text style={[styles.questFooter,{color:t.muted}]}>{Math.round(ratio*100)}% COMPLETE</Text>
              </View>;
            })}
          </View>
        ...
        </> : <View style={[styles.empty,{borderColor:t.border,backgroundColor:t.surface}]}><IconArt name={tab==='Completed'?'trophy':'quest'} size={62}/><Text style={[styles.emptyTitle,{color:t.text}]}>{tab==='Completed'?'Quest history':'Guild campaigns'}</Text><Text style={[styles.emptyCopy,{color:t.muted}]}>{tab==='Completed'?'Completed quests will become a permanent progression ledger with rewards and dates earned.':'Co-op boss battles and shared quest chains are reserved for the social phase.'}</Text></View>}
      </View>
    </ScrollView>
  </SafeAreaView>;
}
