...
export async function recordCoachingOutcome(db:SQLiteDatabase,args:{sessionId:string;actualTemplateId:string;actualOutput:number;context?:CoachingContext}){
  ...
  const followed=recommended===args.actualTemplateId;let outcomeScore=scoreAgainstBaseline(Math.max(0,args.actualOutput),baseline);
...
export async function captureLatestCoachingOutcome(db:SQLiteDatabase){
  ...
  const result=await recordCoachingOutcome(db,{sessionId:latest.id,actualTemplateId:latest.template_id,actualOutput,context:{recommendedTemplateId:remembered.recommendedTemplateId,priority:remembered.priority,readiness:remembered.readiness,confidence:remembered.confidence,source:remembered.source}});
