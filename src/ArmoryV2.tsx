...
const art:Record<ThemeId,string>={
  mythicForge:'/vitalquest-app/art/premium/mythic-hero.jpg',
...
function RewardGrid({tab,wide}:{tab:Exclude<Tab,'Worlds'>;wide:boolean}){
  ...
  return <View><View style={styles.sectionHead}><View><Text style={[styles.eyebrow,{color:t.accent}]}>{tab.toUpperCase()} VAULT</Text><Text style={[styles.sectionTitle,{color:t.text}]}>Collected rewards</Text></View><Text style={[styles.count,{color:t.muted}]}>{items.length} ITEMS</Text></View><View style={[styles.grid,wide&&styles.gridWide]}>{items.map((item,index)=><View key={item.name} style={[styles.item,{borderColor:t.border,backgroundColor:t.surface},item.state==='locked'&&{opacity:.48}]}><View style={styles.itemTop}><IconArt name={index%2===0?'armory':'trophy'} size={54}/><Text style={[styles.state,{color:item.state==='unlocked'?t.accent:t.muted}]}>{item.state==='unlocked'?'UNLOCKED':item.state==='progress'?'IN PROGRESS':'LOCKED'}</Text></View><Text style={[styles.kind,{color:t.muted}]}>{item.kind.toUpperCase()}</Text><Text style={[styles.name,{color:t.text}]}>{item.name}</Text><Text style={[styles.itemCopy,{color:t.muted}]}>{item.state==='unlocked'?'Earned through real-world progression.':item.state==='progress'?'Keep training to complete this unlock.':'Requirement hidden until the previous tier is cleared.'}</Text></View>)}</View></View>
}
