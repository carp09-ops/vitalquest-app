...
export async function getProgressionSnapshot(db:SQLiteDatabase):Promise<ProgressionSnapshot>{
  ...
  const [totalXP,strengthXP,staminaXP,agilityXP,vitalityXP,disciplineXP,workoutCount,enduranceCount,recoveryCount,totalVolume,lifetimeMiles,prCount,thisWeekWorkouts,thisWeekEnduranceSessions,thisWeekRecoverySessions,thisWeekVolume,thisWeekMiles,thisWeekXP]=await Promise.all([
    getNumber(db,`SELECT COALESCE(SUM(amount),0) AS total FROM xp_events`),getNumber(db,`SELECT COALESCE(SUM(amount),0) AS total FROM attribute_events WHERE attribute='strength'`),getNumber(db,`SELECT COALESCE(SUM(amount),0) AS total FROM attribute_events WHERE attribute='stamina'`),getNumber(db,`SELECT COALESCE(SUM(amount),0) AS total FROM attribute_events WHERE attribute='agility'`),getNumber(db,`SELECT COALESCE(SUM(amount),0) AS total FROM attribute_events WHERE attribute='vitality'`),getNumber(db,`SELECT COALESCE(SUM(amount),0) AS total FROM attribute_events WHERE attribute='discipline'`),getNumber(db,`SELECT COUNT(*) AS total FROM workout_sessions`),getNumber(db,`SELECT COUNT(*) AS total FROM workout_sessions WHERE template_id='run'`),getNumber(db,`SELECT COUNT(*) AS total FROM workout_sessions WHERE template_id='recovery'`),getNumber(db,`SELECT COALESCE(SUM(total_volume),0) AS total FROM workout_sessions`),getNumber(db,`SELECT COALESCE(SUM(distance_miles),0) AS total FROM endurance_sessions`),getNumber(db,`SELECT COUNT(*) AS total FROM exercise_sets WHERE is_pr=1`),getNumber(db,`SELECT COUNT(*) AS total FROM workout_sessions WHERE completed_at >= ?`,weekStart),getNumber(db,`SELECT COUNT(*) AS total FROM workout_sessions WHERE template_id='run' AND completed_at >= ?`,weekStart),getNumber(db,`SELECT COUNT(*) AS total FROM workout_sessions WHERE template_id='recovery' AND completed_at >= ?`,weekStart),getNumber(db,`SELECT COALESCE(SUM(total_volume),0) AS total FROM workout_sessions WHERE completed_at >= ?`,weekStart),getNumber(db,`SELECT COALESCE(SUM(distance_miles),0) AS total FROM endurance_sessions WHERE created_at >= ?`,weekStart),getNumber(db,`SELECT COALESCE(SUM(total_xp),0) AS total FROM workout_sessions WHERE completed_at >= ?`,weekStart),
  ]);
