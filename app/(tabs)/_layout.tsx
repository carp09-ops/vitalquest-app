...
function NavIcon({ name, focused, accent }: { name: keyof typeof NAV; focused: boolean; accent: string }) {
  ...
  return <View style={[styles.iconShell, focused && { backgroundColor: `${accent}14`, borderColor: `${accent}52` }]}><IconArt name={NAV[name]} size={34} opacity={focused ? 1 : .54} tint={focused ? accent : t.muted} quiet={!focused} />{focused ? <View style={[styles.activePip, { backgroundColor: accent }]} /> : null}</View>;
}

export default function TabLayout() {
  const db=useSQLiteContext();const { theme } = useVitalTheme();const { archetype } = useHeroArchetype();const { width } = useWindowDimensions();const [gate,setGate]=useState<'checking'|'open'>('checking');const landscapeDock = width >= 900;const t = theme.tokens;const accent = paletteForArchetype(archetype).primary;
