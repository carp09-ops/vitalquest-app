  ...
  return <SafeAreaView style={[styles.safe,{backgroundColor:'transparent'}]}>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[styles.page,wide&&styles.pageWide]}>
      <View style={styles.shell}>
        ...
        <View style={[styles.heroCard,{backgroundColor:t.heroSurface,borderColor:t.border},web({boxShadow:'0 24px 60px rgba(0,0,0,.34)'})]}>
          <View style={{flex:1}}>
            <Text style={[styles.eyebrow,{color:t.accent}]}>TODAY</Text>
            <Text style={[styles.title,{color:t.text}]}>Keep building.</Text>
            <Text style={[styles.copy,{color:t.muted}]}>Your training feeds XP, attributes, quests, and the hero evolution system.</Text>
            <View style={styles.xpRow}><Text style={[styles.xpMain,{color:t.text}]}>{snapshot.totalXP.toLocaleString()} XP</Text><Text style={[styles.xpSub,{color:t.muted}]}>{snapshot.levelCurrentXP.toLocaleString()} / {snapshot.levelNeededXP.toLocaleString()} to next level</Text></View>
            <View style={[styles.track,{backgroundColor:t.surfaceElevated}]}><View style={[styles.fill,{width:`${Math.min(100,snapshot.levelRatio*100)}%`,backgroundColor:t.accent}]}/></View>
          </View>
          <IconArt name="xp" size={58}/>
        </View>
...
const styles=StyleSheet.create({
  ...
  heroCard:{borderWidth:1,borderRadius:22,padding:20,flexDirection:'row',alignItems:'center',gap:18},eyebrow:{fontSize:7,fontWeight:'900',letterSpacing:1.4},title:{fontSize:34,fontWeight:'900',letterSpacing:-1.2,marginTop:5},copy:{fontSize:10,lineHeight:16,marginTop:7,maxWidth:620},xpRow:{flexDirection:'row',alignItems:'baseline',gap:10,marginTop:18,flexWrap:'wrap'},xpMain:{fontSize:20,fontWeight:'900'},xpSub:{fontSize:8.5,fontWeight:'700'},track:{height:7,borderRadius:99,overflow:'hidden',marginTop:8},fill:{height:'100%',borderRadius:99},
