...
async function writeSessionBase(db: SQLiteDatabase, input: BaseSession) {
  ...
  for (const gain of attributeGains) {
    ...
    await db.runAsync(`INSERT INTO attribute_events (id, session_id, attribute, amount, reason, created_at) VALUES (?, ?, ?, ?, ?, ?)`,`${input.sessionId}-${gain.attribute}`,input.sessionId,gain.attribute,gain.amount,`${gain.reason}:${trust.tier.toLowerCase()}`,input.completedAt);
  }
...
export async function saveCompletedWorkout(db: SQLiteDatabase,input: BaseSession & {sets:Array<{id:string;exerciseId:string;exerciseName:string;setNumber:number;weight:number;reps:number;isPR:boolean;completedAt?:string;}>;}) {
  let trust:ReturnType<typeof evaluateXPTrust>|null=null;
  await db.withTransactionAsync(async()=>{
    trust=await writeSessionBase(db,{...input,verificationEvidence:{completedUnits:input.sets.length,totalVolume:input.totalVolume,...input.verificationEvidence},syncDetails:{...input.syncDetails,sets:input.sets}});
    for(const set of input.sets){await db.runAsync(`INSERT INTO exercise_sets (id, session_id, exercise_id, exercise_name, set_number, weight, reps, is_pr, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,set.id,input.sessionId,set.exerciseId,set.exerciseName,set.setNumber,set.weight,set.reps,set.isPR?1:0,set.completedAt??input.completedAt);}
  });
  return trust!;
}

export async function saveCompletedEnduranceSession(db: SQLiteDatabase,input: BaseSession & {distanceMiles:number;avgPaceSeconds:number;activityType?:'run'|'walk'|'bike'|string;}) {
  let trust:ReturnType<typeof evaluateXPTrust>|null=null;
  await db.withTransactionAsync(async()=>{
    trust=await writeSessionBase(db,{...input,verificationEvidence:{modality:'endurance',completedUnits:1,distanceMiles:input.distanceMiles,avgPaceSeconds:input.avgPaceSeconds,...input.verificationEvidence},syncDetails:{...input.syncDetails,distanceMiles:input.distanceMiles,avgPaceSeconds:input.avgPaceSeconds,activityType:input.activityType??'run'}});
    await db.runAsync(`INSERT INTO endurance_sessions (id, session_id, distance_miles, duration_minutes, avg_pace_seconds, activity_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,`${input.sessionId}-endurance`,input.sessionId,input.distanceMiles,input.durationMinutes,input.avgPaceSeconds,input.activityType??'run',input.completedAt);
  });
  return trust!;
}
...
export async function getEquipmentProfile(db: SQLiteDatabase): Promise<EquipmentId[]> {
  ...
  try { const parsed=JSON.parse(row.value_json); return Array.isArray(parsed)&&parsed.length?parsed:DEFAULT_EQUIPMENT; } catch { return DEFAULT_EQUIPMENT; }
}
