...
function createWebDb(){return{
  ...
  async getFirstAsync<T=any>(sql:string,...params:any[]):Promise<T|null>{const store=loadStore();const normalized=normalize(sql);
    if(normalized.includes('from app_preferences')){const literal=normalized.match(/key\s*=\s*['"]([^'"]+)['"]/i)?.[1];const key=params[0]??literal;if(key){const row=store.app_preferences.find(row=>row.key===key);return(row?{value_json:row.value_json}:null) as T|null}}
    ...
    if(normalized.includes('from session_verification')&&normalized.includes('order by created_at desc')){const row=[...store.session_verification].sort((a,b)=>String(b.created_at).localeCompare(String(a.created_at)))[0];return(row??null) as T|null}
    return null;
  },
