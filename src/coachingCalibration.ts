...
export type CoachingCalibration={
  observations:number;
  followed:number;
  followRate:number;
  validated:number;
  calibrationScore:number;
  label:'LEARNING'|'CALIBRATING'|'PERSONALIZED';
  pattern:string;
};
...
function scoreAgainstBaseline(actual:number,baseline:number){
  if(baseline<=0)return 75;
  const ratio=actual/baseline;
  if(ratio>=1.05)return 100;
  if(ratio>=.95)return 90;
  if(ratio>=.85)return 75;
  if(ratio>=.7)return 55;
  return 35;
}

export async function recordCoachingOutcome(db:SQLiteDatabase,args:{sessionId:string;actualTemplateId:string;actualOutput:number;context?:CoachingContext}){
  const recommended=args.context?.recommendedTemplateId;
  ...
  const followed=recommended===args.actualTemplateId;
  let outcomeScore=scoreAgainstBaseline(Math.max(0,args.actualOutput),baseline);
...
export async function getCoachingCalibration(db:SQLiteDatabase):Promise<CoachingCalibration>{
  await ensureTable(db);
  const rows=await db.getAllAsync<{recommended_template_id:string;actual_template_id:string;followed:number;outcome_score:number;outcome_label:string}>(`SELECT recommended_template_id,actual_template_id,followed,outcome_score,outcome_label FROM coaching_outcomes ORDER BY created_at DESC LIMIT 30`);
  ...
  const followed=rows.filter(row=>row.followed===1).length;
  const validated=rows.filter(row=>row.outcome_label==='VALIDATED').length;
  const calibrationScore=Math.round(rows.reduce((sum,row)=>sum+Number(row.outcome_score||0),0)/observations);
  const followRate=Math.round((followed/observations)*100);
  const label=observations>=8&&calibrationScore>=75?'PERSONALIZED':observations>=3?'CALIBRATING':'LEARNING';
  const skips=new Map<string,number>();
  for(const row of rows){if(row.followed===0)skips.set(row.recommended_template_id,(skips.get(row.recommended_template_id)??0)+1)}
  const topSkip=[...skips.entries()].sort((a,b)=>b[1]-a[1])[0];
