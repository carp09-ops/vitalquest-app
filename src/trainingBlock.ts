...
async function ensureBlockStart(db:SQLiteDatabase){
  const existing=await getPref(db,'training_block_v1');
  if(existing?.startAt){
    const start=new Date(existing.startAt);
    const age=Date.now()-start.getTime();
    if(age<TOTAL_WEEKS*WEEK)return start;
  }
  const first=await db.getFirstAsync<{completed_at:string}>(`SELECT completed_at FROM workout_sessions ORDER BY completed_at DESC LIMIT 1`);
  const start=mondayStart(first?.completed_at?new Date(first.completed_at):new Date());
  await setPref(db,'training_block_v1',{startAt:start.toISOString(),totalWeeks:TOTAL_WEEKS});
  return start;
}

export async function getTrainingBlock(db:SQLiteDatabase,intelligence?:TrainingIntelligence):Promise<TrainingBlock>{
  let start=await ensureBlockStart(db);
  let elapsed=Math.max(0,Date.now()-start.getTime());
  if(elapsed>=TOTAL_WEEKS*WEEK){
    start=mondayStart(new Date());
    await setPref(db,'training_block_v1',{startAt:start.toISOString(),totalWeeks:TOTAL_WEEKS});
    elapsed=Math.max(0,Date.now()-start.getTime());
  }
  const week=Math.min(TOTAL_WEEKS,Math.floor(elapsed/WEEK)+1);
  const config=phases[week];
  const weekStart=new Date(start.getTime()+(week-1)*WEEK);
  const weekEnd=new Date(weekStart.getTime()+WEEK);
  const endAt=new Date(start.getTime()+TOTAL_WEEKS*WEEK);
  ...
  const resistanceCompleted=rows.filter(row=>row.template_id!=='run'&&row.template_id!=='recovery').length;
  const enduranceCompleted=rows.filter(row=>row.template_id==='run').length;
  const recoveryCompleted=rows.filter(row=>row.template_id==='recovery').length;
  const weeklyCompleted=rows.length;
  const blockSessions=blockRows.length;
  const blockXP=blockRows.reduce((sum,row)=>sum+Number(row.total_xp||0),0);
  const trend=intelligence?.performanceTrend??'LIMITED DATA';
  const fatigue=intelligence?.fatigueScore??0;
